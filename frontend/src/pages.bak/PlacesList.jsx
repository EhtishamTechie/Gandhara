...
const PlacesList = () => {
  ...
  const fetchVisitPlaces = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/visit-places/all`);
  ...
  const handleVisitDelete = async (id, name) => {
    if (window.confirm(`Are you sure you want to delete "${name}"? This action cannot be undone.`)) {
      try {
        const response = await fetch(`${API_BASE_URL}/visit-places/${id}`, {
          method: 'DELETE'
        });
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/visit-places/${editingPlace._id}`, {
        method: 'PUT',
        body: formData
      });
