...
// Get API URL from environment variables (fallback for local dev)
const API_BASE_URL = import.meta.env.VITE_API_URL || 'http://127.0.0.1:5000';

const DashboardHome = ({ onNavigateTab }) => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch total count via admin endpoint (limit=5 gives us recent items + total in one call)
      const productsResponse = await fetch(`${API_BASE_URL}/api/admin/products?page=1&limit=5`, { headers });
      let totalProducts = 0;
      let recentProducts = [];
      ...
      // Fetch places
      const placesResponse = await fetch(`${API_BASE_URL}/api/visit-places/all`);
      let places = [];
      if (placesResponse.ok) {
        const placesData = await placesResponse.json();
        places = Array.isArray(placesData) ? placesData : (placesData?.visitPlaces || []);
      }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md border border-stone-200 p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <button
            type="button"
            onClick={() => onNavigateTab?.('add-place')}
          ...
          <button
            type="button"
            onClick={() => onNavigateTab?.('product-list')}
          ...
          <button
            type="button"
            onClick={() => onNavigateTab?.('places-list')}
