...
const PlacesList = () => {
  ...
  const fetchVisitPlaces = async (pageNum = 1, resetPlaces = true) => {
    try {
      if (pageNum === 1) {
        setLoading(true);
        setError(null);
      ...
      const response = await fetch(`${API_BASE_URL}/api/visit-places?page=${pageNum}&limit=${ITEMS_PER_PAGE}`);
      ...
      if (Array.isArray(data)) {
        placesArray = data;
      } else if (data && Array.isArray(data.visitPlaces)) {
        placesArray = data.visitPlaces;
      ...
      if (resetPlaces || pageNum === 1) {
        setVisitPlaces(placesArray.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
      } else {
        setVisitPlaces(prev => [...prev, ...placesArray.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))]);
      }
      
      setTotal(data.total || placesArray.length);
      setHasMore((placesArray?.length || 0) === ITEMS_PER_PAGE && (placesArray?.length || 0) > 0);
      setPage(pageNum);
    } catch (err) {
      console.error("Error loading visit places:", err);
      setError(err.message);
      if (pageNum === 1) setVisitPlaces([]);
    } finally {
      setLoading(false);
  ...
  const handleVisitEdit = (place) => {
    ...
    setEditForm({
      ...
      image: null,
      imagePreview: place.image ? getImageUrl(place.image) : null
    });
      ...
      {!loading && !error && (
        <div className="text-center py-8">
          ...
          {!loadingMore && hasMore && (
            <button
              onClick={loadMore}
              className="bg-stone-600 hover:bg-stone-700 text-white px-6 py-3 rounded-lg font-medium transition-colors duration-200"
            >
              Load More Places
            </button>
          )}
          
          {!hasMore && visitPlaces.length > 0 && (
            <p className="text-stone-600">All places loaded ({total} total)</p>
          )}
        </div>
      )}
